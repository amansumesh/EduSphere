      ...
      <div className="flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-20 pt-10 text-left">
        ...
        <div className="max-w-xl z-10 text-gray-300">
          ...
          <div className='flex items-center space-x-2 pt-3 pb-1 text-sm text-gray-300'>
            ...
            <p className='text-brand-purple-400'>({courseData.courseRatings.length} {courseData.courseRatings.length > 1 ? 'ratings' : 'rating'})</p>

            <p>{courseData.enrolledStudents.length} {courseData.enrolledStudents.length > 1 ? 'students' : 'student'}</p>
          </div>

          <p className='text-sm text-gray-300'>Course by <span className='text-brand-purple-400 underline'>{courseData.educator.name}</span></p>
        ...
        <div className="max-w-course-card z-10 shadow-custom-card rounded-t md:rounded-none overflow-hidden bg-white/5 border border-white/10 min-w-[300px] sm:min-w-[420px]">
          ...
          <div className="p-5">
            <div className="flex items-center gap-2">
              ...
              <p className="text-brand-pink-400">
                <span className="font-medium">5 days</span> left at this price!
              </p>
            </div>
            <div className="flex gap-3 items-center pt-2">
              <p className="text-white md:text-4xl text-2xl font-semibold">{currency}{(courseData.coursePrice - courseData.discount * courseData.coursePrice / 100).toFixed(2)}</p>
            ...
            <button onClick={enrollCourse} className="md:mt-6 mt-4 w-full py-3 rounded bg-gradient-to-r from-brand-pink-500 to-brand-purple-500 hover:from-brand-pink-600 hover:to-brand-purple-600 text-white font-medium">
